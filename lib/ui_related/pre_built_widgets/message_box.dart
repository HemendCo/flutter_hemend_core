...
class MessageBoxSegment extends CustomPainter {
  ...
  final Paint? painter;
  MessageBoxSegment(this.radius, this.sharpPointSize,
  ...
  void paint(Canvas canvas, Size size) {
    Paint canvasPainter = Paint();
    if (painter != null) {
      canvasPainter = painter!;
    } else {
      canvasPainter.color = const Color(0xff447a9c);
      canvasPainter.maskFilter = MaskFilter.blur(BlurStyle.normal, 1);
      canvasPainter.style = PaintingStyle.stroke;
    }
    final path = MessageBoxSegment.messageBox(size.width, size.height, radius,
        sharpPointSize, sharpPointAtEnd, sharpPointOffset, sharpPointPath);
    canvas.drawPath(path, canvasPainter);
  }

  static Path messageBox(double width, double height,
    ...
    Path sharpPoint = Path();

    ///Creating Base RoundRect
    path.addRRect(RRect.fromRectAndRadius(
        Rect.fromLTRB(0, 0, boxWidth, boxHeight), borderRadius));
    ...
    if (sharpPointAtEnd) {
      path = path
          .transform(Matrix4.diagonal3Values(-1, -1, 0).storage)
          .shift(Offset(boxWidth, boxHeight));
    }
    ...
    path = path.transform(finalScaleMatrix.storage);
